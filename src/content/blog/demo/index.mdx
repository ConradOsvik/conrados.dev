---
title: 'Demo of markdown features'
description: 'This is a quick demo of markdown features'
date: '2025-09-19'
cover: './cover.jpg'
tags: ['blog', 'markdown']
---

## Introduction

This is the introduction to my first blog. Let me show you what we can do with `inline code` and other markdown features.

> This is a blockquote to demonstrate the styling. It should have nice borders and muted text colors that work well with the shadcn design system.

## Images

A regular image with alt text:

![A placeholder landscape photo](./cover.jpg)

An image with an optional caption (use the markdown title attribute):

![A placeholder landscape photo](./cover.jpg "The cover photo — stored right next to this MDX file")

## Lists and Text

Here's an unordered list:

- First item with some text
- Second item that's longer to show how wrapping works in the list items
- Third item

And an ordered list:

1. First numbered item
2. Second numbered item with `inline code`
3. Third item with a [link](https://example.com)

## Tables

| Feature        | Status | Notes              |
| -------------- | ------ | ------------------ |
| Basic markdown | ✅     | All implemented    |
| Code blocks    | ✅     | With line numbers  |
| Math           | ✅     | KaTeX support      |
| Tables         | ✅     | Responsive styling |

## Code Examples

Here's a TypeScript example with line numbers:

```ts title="calculator.ts" showLineNumbers mark={1-3,5} ins={10-12} del={15-17}
console.log('Hello, world!')

function add(a: number, b: number): number {
    return a + b
}

class Calculator {
    constructor(private initialValue: number = 0) {}

    add(value: number): this {
        this.initialValue += value
        return this
    }

    getValue(): number {
        return this.initialValue
    }
}

const calc = new Calculator(10).add(5).add(3)

console.log(calc.getValue()) // 18
```

## Math Examples

Inline math like $E = mc^2$ should be styled nicely.

Block math gets special treatment:

$$
\int_0^1 x^2 \, dx = \frac{1}{3}
$$

More complex math:

$$
f(x) = ax^2 + bx + c
$$

## GitHub Flavored Markdown (GFM)

### Task Lists

- [x] Implement feature
- [ ] Write tests
    - [x] Unit tests
    - [ ] Integration tests
- [ ] Update docs

### Strikethrough

This text has ~~obsolete information~~ that was updated later.

### Autolink Literals

Bare links are automatically linked in GFM:

https://example.com and www.github.com will both be clickable.

### Footnotes

Here is a statement with a footnote.[^note]

[^note]: This is the footnote content. You can include links like https://github.com and even more text.

### Aligned Table

| Name    | Score | Trend |
| :------ | ----: | :---: |
| Alice   |   100 |  ⬆️   |
| Bob     |    75 |  ⬇️   |
| Charlie |    90 |   ⟷   |

### Diff Code Block

```diff
diff --git a/app/page.tsx b/app/page.tsx
index 1111111..2222222 100644
--- a/app/page.tsx
+++ b/app/page.tsx
@@
- return <h1>Hello</h1>
+ return <h1>Hello, world!</h1>
```

### Details Disclosure

<details>
    <summary>Click to expand extra context</summary>
    <div>
        This section uses native HTML details/summary which renders well on
        GitHub and most MDX renderers.
        <ul>
            <li>It can contain lists</li>
            <li>And <code>inline code</code></li>
        </ul>
    </div>
</details>

## Conclusion

This demonstrates all the styling features working together with the design system!
